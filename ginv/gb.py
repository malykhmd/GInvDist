...
class GB(list):
  ...
  def algorithm11(self, F, output=False):
    ...
    #self.F, self.B = [Poly(p.copy()) for p in F], []
    self.F, self.B = F, []
    while F or self.B:
      p, i = None, 0
      while i < len(F):
        if output: print(i, F[i].lm(),end="")
        F[i].NFhead(self)
        if output: print('->', F[i].lm() if F[i] else '0')
        if not F[i]:
          del F[i]
        else:
          F[i].pp()
  ...
  def algorithm1(self, F, output=False):
    ...
    super(GB, self).__init__()
    self.time, self.crit1, self.crit2 = time.time(), 0, 0,
    self.F, self.B = F, [] #[p.copy() for p in F], []
    while F or self.B:

      p, i = None, 0
      while i < len(F):
        if output: print(i, F[i].lm(),end="")
        F[i].NFhead(self) # Change 
        if output: print('->', F[i].lm() if F[i] else '0')
        if not F[i]: # Change 
          del F[i] # Change 
        else:
